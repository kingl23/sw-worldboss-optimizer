...
def build_opp_guild_options(rows: Iterable[dict]) -> List[str]:
    s = set()
    for r in rows:
...
def build_defense_deck_stats(
    rows: Iterable[dict],
    limit: int = 50,
    guild_groups: Dict[str, set] | None = None,
) -> pd.DataFrame:
    """
    (항상 mode=1 포맷)
    반환 컬럼:
    ...
    groups = guild_groups or GUILD_GROUPS
    ...
    for r in rows:
        ...
        if og:
            if og in groups["in32"]:
                _inc_pair(g32, def_key, is_win)
            if og in groups["in12"]:
...
def build_defense_decks_vs_guild(
    rows: Iterable[dict],
    opp_guild: str,
    limit: int = 50,
) -> pd.DataFrame:
