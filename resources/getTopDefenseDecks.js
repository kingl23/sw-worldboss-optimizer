/**
 * 선택한 길드원(wizardName)의 방덱을 집계해서
 * 상위 limit개까지 [몬1, 몬2, 몬3, 승, 패, 승률%] 형태로 반환
 *
 * 시트에서 예: =getTopDefenseDecks($C$2, 10)
 */
function getTopDefenseDecks(wizardName, limit) {
  ...
  const sheet = ss.getSheetByName("DefenseLogs");
  if (!sheet) {
    return [["DefenseLogs 시트를 찾을 수 없습니다.", "", "", "", "", ""]];
  }
  ...
  const list = keys.map(k => {
    ...
    return {
      ...
      total: total,
      winRate: winRate,
      winRatePercent: (winRate * 100).toFixed(1) + "%", // 소수점 1자리 % 표시
      key: k
    };
  });
