...
def make_off_key(a, b, c) -> str:
    """Build an offense key with a fixed leader and sorted followers."""
    A = _norm(a)
    B = _norm(b)
    C = _norm(c)
    if not A:
...
def cumulative_trend_df(siege_df: pd.DataFrame) -> pd.DataFrame:
    """Return a cumulative trend frame for the siege chart."""
...
build_cumulative_trend_df = cumulative_trend_df
