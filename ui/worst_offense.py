...
def render_worst_offense_tab():
    ...
    with st.form("worst_off_form"):
        ...
        top_n  = col2.number_input("Rows to show (Top N)", 1, 200, 50, 5, key="wo_top_n")
    ...
    off_display = off_df.rename(
        columns={"wins": "Wins", "losses": "Losses", "total": "Total Games"},
        errors="ignore",
    )
    st.dataframe(
        off_display[["Unit #1", "Unit #2", "Unit #3", "Wins", "Losses", "Win Rate", "Summary", "Total Games"]],
        use_container_width=True,
        hide_index=True,
    )
