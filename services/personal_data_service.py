...
def get_record_summary(wizard_name: str) -> pd.DataFrame:
    ...
    rows = []
    for label, key in [("Overall", "all"), ("4-star", "four"), ("5-star", "five")]:
        win = totals[key]["win"]
...
def get_offense_deck_details(wizard_name: str, offense_key: str, limit: int) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        mapped_rows.append(
            {
                "Offense Unit #1": _get_col(row, "deck1_1"),
                "Offense Unit #2": _get_col(row, "deck1_2"),
                "Offense Unit #3": _get_col(row, "deck1_3"),
                "Defense Unit #1": _get_col(row, "deck2_1"),
                "Defense Unit #2": _get_col(row, "deck2_2"),
                "Defense Unit #3": _get_col(row, "deck2_3"),
